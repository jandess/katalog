    ...
    card.innerHTML = `
      <div class="relative aspect-[3/4] overflow-hidden">
        ...
        <button onclick="toggleFavorite(${p.id}, event)" class="absolute top-2 right-2 w-10 h-10 flex items-center justify-center bg-white/90 backdrop-blur-md rounded-full text-error transition-all active:scale-125 md:opacity-100 md:group-hover:opacity-100">
          <span class="material-symbols-outlined text-[24px] ${isFav ? 'fill' : ''}">favorite</span>
        </button>
      </div>
    ...
    card.innerHTML = `
      <div class="relative aspect-square overflow-hidden">
        ...
        <button onclick="toggleFavorite(${p.id}, event)" class="absolute top-2 right-2 w-9 h-9 flex items-center justify-center bg-white/90 backdrop-blur-md rounded-full text-error">
          <span class="material-symbols-outlined text-[22px] fill">favorite</span>
        </button>
      </div>
      <div class="p-4">
      ...
      <div class="flex flex-col justify-between flex-grow">
        ...
        <div class="flex justify-between items-center mt-2">
          ...
          <div class="flex items-center bg-surface-container-low rounded-full px-2 border border-outline-variant/20">
            <button onclick="updateCartQty(${index}, -1)" class="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center text-primary"><span class="material-symbols-outlined text-[18px]">remove</span></button>
            <span class="px-2 font-semibold text-sm md:text-base w-6 md:w-8 text-center">${item.qty}</span>
            <button onclick="updateCartQty(${index}, 1)" class="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center text-primary"><span class="material-symbols-outlined text-[18px]">add</span></button>
          </div>
        </div>
      </div>
    `;
...
function renderPackagingTypes() {
  ...
  packagingTypes.forEach(type => {
    ...
    btn.className = `packaging-type-btn flex-none px-4 py-2.5 rounded-lg border-2 transition-all snap-start whitespace-nowrap font-label-md text-xs ${isSelected ? 'border-secondary bg-secondary-container text-on-secondary-container font-bold shadow-sm' : 'border-outline-variant/30 bg-white text-on-surface-variant'}`;
    btn.onclick = () => setBox(type.id_tipe);
    btn.dataset.type = type.id_tipe;
    const priceText = type.harga_tambahan === 0 ? 'Tanpa biaya tambahan' : `+${type.harga_tambahan.toLocaleString('id-ID')}/item`;
    btn.innerHTML = `${type.id_tipe}<br><span class="text-[9px] font-normal">${priceText}</span>`;
    container.appendChild(btn);
  });
...
function setBox(type) {
  ...
  document.querySelectorAll('.packaging-type-btn').forEach(btn => {
    if (btn.dataset.type === type) {
      btn.classList.add('border-secondary', 'bg-secondary-container', 'text-on-secondary-container', 'font-bold', 'shadow-sm');
      btn.classList.remove('border-outline-variant/30', 'bg-white', 'text-on-surface-variant');
    } else {
      btn.classList.add('border-outline-variant/30', 'bg-white', 'text-on-surface-variant');
      btn.classList.remove('border-secondary', 'bg-secondary-container', 'text-on-secondary-container', 'font-bold', 'shadow-sm');
    }
  });
...
function renderVariantGrid(type) {
  ...
  variants.forEach(variant => {
    ...
    const btn = document.createElement('button');
    btn.className = `flex-none px-4 py-2.5 rounded-lg border-2 transition-all snap-start whitespace-nowrap font-label-md text-xs ${isSelected ? 'border-secondary bg-secondary-container text-on-secondary-container font-bold shadow-sm' : 'border-outline-variant/30 bg-white text-on-surface-variant'}`;
